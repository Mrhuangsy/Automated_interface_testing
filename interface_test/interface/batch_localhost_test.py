...
class BatchLocalhostTest(unittest.TestCase):
    '''供应链移动端接口测试'''
    def setUp(self):
        self.base_url = readConfig.get_http('localhosturl')
...
def __generateTestCases():
    print("黄思远")
    token = otherutils.getToken_net('localhosturl')
    print('token:',token)
    usecase = otherutils.read_json_file(parentdir+'/datas/usecase_localhost.json')
    arglists=[]
    ...
    for args in arglists:
        setattr(BatchLocalhostTest,'test_func_%s'%(flag+args[4]),
                BatchLocalhostTest.getTestFunc(*args))
__generateTestCases()
