...
class BatchCrmTest(unittest.TestCase):
    '''客户关系管理移动端接口测试'''
    def setUp(self):
...
def __generateTestCases():
    ...
    print('token:',token)
    usecase = otherutils.read_json_file(parentdir+'/datas/usecase_crm.json')
    arglists=[]
    ...
    for args in arglists:
        setattr(BatchCrmTest,'test_func_%s'%(flag+args[4]),
                BatchCrmTest.getTestFunc(*args))
__generateTestCases()
